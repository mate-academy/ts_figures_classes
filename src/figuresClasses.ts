...
export class Triangle implements Figure {
  ...
  constructor(
    ...
    if (a <= 0 || b <= 0 || c <= 0) {
      throw new Error('Sides of triangle must be bigger than 0');
  ...
  getArea(): number {
    ...
    return Math.trunc(Math.sqrt(halfOfPerimeter
...
export class Rectangle implements Figure {
  ...
  constructor(
    ...
    if (width <= 0 || height <= 0) {
      throw new Error('Size of each side must be bigger than 0');
    }
  }
